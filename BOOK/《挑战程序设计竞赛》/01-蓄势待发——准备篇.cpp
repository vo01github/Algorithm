...
2.O(n^2logn)的算法	
	...
	这种情况并不能直接使用二分搜索。但是，如果预先枚举出 kc + kd 的所有可能的数字 并排好序，便可以利用二分搜索了。

	排序O(n^2logn)时间
	循环O(n^2logn)时间
	总的也是 O(n^2logn) 时间，这样就可以确信即便 n=1000 也能妥善应对了。
*/
...
binary_search( m - k[a] - k[b]  );			// 将内侧的两个循环替换成二分搜索

/*
	本问题既需要二分搜索这一基础算法知识，也需要将四个数分成两两考虑的想象力。
	此外，像这样从复杂度较高的算法出发，不断降低复杂度直到满足问题需求的过程，也是设计算法时常会经历的一个过程。
*/